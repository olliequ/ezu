...
"""A view is something that executes when a URL is hit. If it's a GET request to the URL,
then the view below will render a HTML file, where we pass to it all the objects
from the Instructor model (passed as a dict -- all arguments will be here)!"""
...
"""The PK specifier added to the URL is accessible in the get request view function.
First we check if the index provided exists in the Instructor table. If it does,
we proceed and pass to the HTML file the specific instructor, and also all of the sections the
instructor has a foreign key in!"""
